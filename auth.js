document.addEventListener("DOMContentLoaded", () => {
  ...
  const isVerificationPage = window.location.pathname.includes("verificar-email.html")
  ...
  if (isLoginPage || isRegisterPage || isVerificationPage) {
    ...
    if (!sessionStorage.getItem("allowAuthPages")) {
      console.log("Estableciendo bandera para permitir páginas de autenticación")
  ...
  if (loginForm) {
    ...
    loginForm.addEventListener("submit", (e) => {
      ...
      if (!user.emailVerified) {
        ...
        showAuthMessage("Por favor, verifica tu correo electrónico primero", "error")
  ...
  if (registerForm) {
    ...
    registerForm.addEventListener("submit", (e) => {
      ...
      // Validar email
      if (!isValidEmail(email)) {
        document.getElementById("email-register-error").textContent = "Por favor, ingresa un correo electrónico válido"
        isValid = false
      } else {
      ...
      if (!tipoUsuario) {
        document.getElementById("tipo-usuario-error").textContent = "Por favor, selecciona un tipo de usuario"
        isValid = false
      } else {
      ...
      // Mostrar mensaje de éxito
      showAuthMessage("¡Registro exitoso! Redirigiendo a verificación de correo...", "success")
  ...
  if (verificationForm) {
    ...
    if (!pendingEmail) {
      console.log("No hay email pendiente de verificación, redirigiendo a registro")
    ...
    verificationForm.addEventListener("submit", (e) => {
      ...
      if (enteredCode.length !== 5) {
        document.getElementById("verification-error").textContent = "Por favor, ingresa el código completo"
        return
      }
      ...
      if (enteredCode === user.verificationCode) {
        ...
        // Mostrar mensaje de éxito
        showAuthMessage("¡Correo verificado con éxito! Redirigiendo al inicio de sesión...", "success")
    ...
    if (resendCodeButton) {
      resendCodeButton.addEventListener("click", (e) => {
        ...
        showAuthMessage("Se ha reenviado un nuevo código a tu correo electrónico", "success")
      })
    }
  }
  ...
  console.log("Estado de sesión:", currentUser ? "Usuario con sesión" : "Sin sesión")
  ...
  if (currentUser && (isLoginPage || isRegisterPage || isVerificationPage)) {
    ...
    if (!sessionStorage.getItem("allowAuthPages")) {
      console.log("Redirigiendo a index.html desde página de autenticación (usuario con sesión)")
      window.location.href = "index.html"
    } else {
      console.log("Permitiendo acceso a página de autenticación a pesar de tener sesión (bandera presente)")
    }
  }
  ...
  if (currentUser) {
    ...
    if (authLinks.length > 0 && userProfileContainer) {
      ...
      console.warn("No se encontraron elementos necesarios para actualizar la interfaz:", {
